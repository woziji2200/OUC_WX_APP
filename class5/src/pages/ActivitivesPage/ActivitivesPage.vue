    ...
    <view>
        ...
        <view class="body">
            <view class="title">
                ...
                <view class="title-content">活动报名</view>
            </view>
            <view class="search">
            ...
            <view class="content">
                <view class="activity" v-for="(item, index) in filteredActivitives" :key="index" @click="NavTo(`/pages/ActivitivesPage/AcDetailPage?id=${item.id}`)">
...
export default {
    ...
    methods: {
        input(e){
            ...
            this.filteredActivitives = this.Activitives.filter(item => item.title.includes(this.keyword) || item.desc.includes(this.keyword))
        },
        NavTo(url) {
            uni.navigateTo({ url })
        },
            ...
            function handleData(data){
                return data.map(item => {
                    return {
                        ...
                        img: BaseUrl + item.photos[0].picture
                    }
                })
            }
            request('/api/news/activity/', 'GET', {}, {}, false).then((res) => {
